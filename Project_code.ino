...
void sendOtpMessage(int genOTP, String number) {
  ...
  SIM900A.println(msg);   // The SMS text you want to send
  delay(100);
...
void verifyCustomer(byte opChoice) {
  ...
  if (fps.IsPressFinger()) {
    ...
    if (id < 5) {
      ...
      stringData = Serial.readString();
      stringData.trim();
      tempotp = stringData.toInt(); // Convert input to integer
        ...
        if (opChoice == 1) {
          ...
          while (Serial.available() > 0) {
            ...
            if (stringData.toInt() > Cstbalance.toInt()) {
              ...
              Cstbalance = String(Cstbalance.toInt() - stringData.toInt());
              //cashDispense();
              Serial.println(F("Collect Your Cash"));
              delay(2000);
              Serial.println(F("Thank You!"));

              Cstbalance.toCharArray(data[id].customer_balance, buflen);
              addrr = addrr + buflen;
